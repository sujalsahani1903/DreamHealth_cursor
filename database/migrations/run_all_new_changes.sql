-- Dream Health Foods — apply all newer schema changes
-- Run in MySQL Workbench, phpMyAdmin, or: mysql -u root -p dream_health_foods < run_all_new_changes.sql
--
-- If a step says "Duplicate column" or "already exists", that part is already applied — skip it and continue.

USE dream_health_foods;

-- =============================================================================
-- 1) Per-product order line status (shipped / pending per item)
-- =============================================================================
...
-- =============================================================================
-- 2) Pack sizes (250g, 500g, 1kg, 2kg, 5kg) per product
-- =============================================================================
...
-- One default 1 kg pack per product (from current product price/stock)
...
-- Optional example packs for product id 1 (Whole Wheat Atta) — safe to re-run
INSERT INTO product_variants (product_id, label, weight_grams, selling_price, cost_price, stock, is_default, sort_order)
...
-- =============================================================================
-- 3) Cart: link each row to a pack size (variant)
-- =============================================================================
...
-- Remove cart rows that could not get a variant (empty cart is fine)
DELETE FROM cart WHERE variant_id IS NULL;

ALTER TABLE cart MODIFY COLUMN variant_id INT UNSIGNED NOT NULL;
...
-- =============================================================================
-- 4) Order items: store pack size on each line
-- =============================================================================
...
-- Done. Restart Flask backend after running this script.
